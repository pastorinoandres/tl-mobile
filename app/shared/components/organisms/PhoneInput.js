...
const CountryPicker = ({currentCountry, items, handleChange, setFieldTouched})=>{

    const flagDefault = require('../../../../assets/flagdefault.png')
    const { code, area } = currentCountry;
    const size = 32
    const source = {uri:`https://www.countryflags.io/${code}/flat/${size}.png`}
    ...
    const onPressCountry = (country)=>{
        handleChange('country')({area:`+${country.phone_code}`, code:country.iso2});
    ...
    const list = items.map((country)=>{
        return {
            ...
            onPress:  ()=>{onPressCountry(country)}
        }
    })
    ...
    return(
        ...
        <TouchableOpacity style={styles.touchable} onPress={toogleMenu}>
            ...
            <Text {...typography["body-18"]} color={colors.grey.t80} extraStyles={{marginHorizontal:5, textAlign:'left'}}>
                {area}
            </Text>
            <DownArrow size={15}/>
...
const PhoneInput = (props)=> {
    ...
    const getError = ()=>{
        ...
        const message =  `${(required === 1)?'Falta un':`Faltan ${required}`} ${(required === 1)?'digito':'digitos'}. Tene en cuenta la caracteristica de tu localidad` 
    ...
    return(
        ...
        <Group style={styles.container} >
            ...
            <Group style={styles.input}>
                ...
                <Line direction='vertical' thickness={calculateSize(0.5)} height={calculateSize(32)} color={colors.grey.t40} marginHorizontal={calculateSize(10)} />
