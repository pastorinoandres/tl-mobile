...
import { ScrollView, View, TextInput, Image, TouchableOpacity, Dimensions, FlatList } from 'react-native';
    ...
    return(
        ...
        <TouchableOpacity style={styles.touchable} onPress={item.onPress}>
            ...
            <View style={styles.row}>
                <Image style={styles.image} source={{uri:item.image}} defaultSource={flagDefault}/>

                <Text {...typography["body-16"]} color={colors.grey.t80} extraStyles={styles.text}>
                    {item.title}
                </Text>  
            </View>
...
const List = (props) =>{
    ...
    return(
        ...
        <Fragment>
            ...
            <Text {...typography["title-20"]} color={colors.acento.primary} extraStyles={styles}>
                {title}
            </Text>
            <SearchInput {...searchProps}/>
            <FlatList
                data={filteredList}
                renderItem={({item})=> <Item title={item.title} image={item.image} onPress={()=>{item.onPress();toDown()}} /> }
...
const Explorer = ({menu, toogleMenu, list, title})=>{
    ...
    return(
        ...
        <BottomMenu menu={menu} toogleMenu={toogleMenu} contentSize={height*0.90} extraStyles={styles}>
            <List {...listProps}/>  
        </BottomMenu>
