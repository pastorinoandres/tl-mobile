...
import { Dimensions, View, TouchableWithoutFeedback, Alert } from 'react-native';
...
const notifications = [
  
    {
      ...
      Icon:require("../../../../../assets/alejandro.jpg"),
      viewed:false
    },
    {
      title:'Mariano Busti',
      message:'Te envio una propuesta para diseño de TuLaburo',
      Icon:require("../../../../../assets/mariano.jpg"),
      viewed:false
    },
    {
      title:'Alejandro Di Luca',
...
const Notifications = ({setHeightTitle, anchor, setAnchor, scrollY, translationX})=>{
    ...
    const gradientColor = (viewed) => (viewed) ? colors.none : colors.notification

    const onLayout = ({nativeEvent: { layout: {x, y, width, height}}}) =>{
      setAnchor(y)
    }

    const onLayoutTitle = ({nativeEvent: { layout: {x, y, width, height}}}) =>{
    ...
    return(
      ...
      <Text {...typography["title-20"]} color={colors.white()} extraStyles={styles.title} animated onLayout={onLayoutTitle} >Notificaciones</Text>
        ...
        {notifications.map( (Option,index) => (
          ...
          >
            <LinearGradient style={styles.border(Option.viewed)} {...gradientColor(Option.viewed)} key={index}>
