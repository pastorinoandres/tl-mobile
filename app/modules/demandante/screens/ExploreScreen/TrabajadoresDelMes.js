...
const TrabajadoresDelMes = () =>{
    ...
    return(
        <View style={styles.mainContainer}>
            <Text {...typography["body-strong-18"]} color={colors.grey.t80}  extraStyles={styles.title}>Trabajadores del mes</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {trabajadores.map((item)=>{
                    return(
                        <View style={styles.cardContainer} key={`${item.name}-${item.skill}`}>
                            <TouchableOpacity style={styles.touchable}>
                                <Image source={item.image} style={styles.image}/>
                                <View style={styles.cardFooter}>
                                    <Text {...typography["body-strong-16"]} color={colors.grey.t80} >{item.name}</Text>                                    
                                    <Text {...typography["body-14"]} color={colors.grey.t60}>{item.skill}</Text>
                                </View>
                            </TouchableOpacity>                            
                        </View>
                    )
