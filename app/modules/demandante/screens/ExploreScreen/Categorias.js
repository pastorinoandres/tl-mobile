    ...
    return(
        <>
            {categorias.map( (categoria,index) =>{
                    ...
                    return(
                        <View style={styles.mainContainer} key={`${categoria.name}-index${index}`}>
                            <Text {...typography["body-strong-18"]} color={colors.grey.t80}  extraStyles={styles.title}>{categoria.name}</Text>
                            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                                {categoria.servicios.map((item, index)=>{
                                    return(
                                        <View style={styles.cardContainer} key={`${index})${item.title}`}>
                                            <TouchableOpacity style={styles.touchable}>
                                                <Image source={item.image} style={styles.image}/>
                                                <View style={styles.cardFooter}>
                                                    <Text {...typography["body-strong-14"]} color={colors.grey.t80} extraStyles={{textAlign:'center'}}>{item.title}</Text>
                                                </View>
                                            </TouchableOpacity>                            
                                        </View>
                                    )
