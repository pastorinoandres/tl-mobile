...
const NuestrasSugerencias = () =>{
    ...
    return(
        <View style={styles.mainContainer}>
            <Text {...typography["body-strong-18"]} color={colors.grey.t80}  extraStyles={styles.title}>Nuestras sugerencias</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {sugerencias.map((item, index)=>{
                    return(
                        <View style={styles.cardContainer} key={`${item.title}-tag${item.tag}-index${index}`}>
                            <TouchableOpacity style={styles.touchable}>
                                <Image source={item.image} style={styles.image}/>
                                <View style={styles.cardFooter}>
                                    <Text {...typography["body-strong-16"]} color={colors.grey.t80} extraStyles={{textAlign:'left'}}>{item.title}</Text>
                                    <Text {...typography["body-14"]} color={colors.grey.t60} extraStyles={{textAlign:'left'}}>{item.subtitle}</Text>
                                </View>
                                <View style={styles.cardTag}>
                                    <Text {...typography["caption-12"]} color={colors.white(1)}>{item.tag}</Text>
                                </View>
                            </TouchableOpacity>                            
                        </View>
                    )
