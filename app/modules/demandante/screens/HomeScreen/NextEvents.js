...
const NextEvents = ()=>{
  ...
  const items = [
    {
      hour:'16:00hs',
      icon: require('../../../../../assets/iconos/jardinero.png'),
      name: 'Corte de pasto'
    },
    {
      hour:'Mañana',
      icon: require('../../../../../assets/iconos/plomero.png'),
      name: 'Cambio de cuerito'
    },
    {
      hour:'Jueves',
      icon: require('../../../../../assets/iconos/marketing.png'),
  ...
  return(
    <Animated.View style={styles.container}>
      <View style={styles.card}>        
        ...
        <Text {...typography["title-20"]} color={colors.grey.t80} extraStyles={{textAlign:'left',marginBottom:30}}>A continuación</Text>
        <Animated.View style={styles.events}>
          {items.map((item,index)=>{
            return(
              <View style={styles.itemContainer} key={`${index}-${item.name}`}>

                <Text {...typography[index?"body-14":"body-strong-14"]} color={colors.grey.t80} extraStyles={{textAlign:'left',width:0.25*windowWidth-ui.padding-4.5}}>{item.hour}</Text>
                <View style={styles.indicator(index)}></View>
                <Thumbnail source={item.icon} size={25} shape='square' extraStyles={{marginHorizontal:20}}/>
                <Text {...typography[index?"body-16":"body-strong-16"]} color={colors.grey.t80} extraStyles={{textAlign:'left'}}>{item.name}</Text>
              </View>
            )
          })}
        </Animated.View>       
